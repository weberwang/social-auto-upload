  ...
  <div class="account-management">
    ...
    <div class="account-tabs">
      <el-tabs v-model="activeTab" class="account-tabs-nav">
        ...
        <el-tab-pane label="微信公众号" name="wechatmp">
          <div class="account-list-container">
            <div class="account-search">
              <el-input
                v-model="searchKeyword"
            ...
            <div v-if="filteredWechatmpAccounts.length > 0" class="account-list">
              <el-table :data="filteredWechatmpAccounts" style="width: 100%">
                <el-table-column label="头像" width="80">
                  <template #default="scope">
                    <el-avatar :src="getDefaultAvatar(scope.row.name)" :size="40" />
                  </template>
            ...
            <div v-else class="empty-data">
              <el-empty description="暂无微信公众号账号数据" />
            </div>
          </div>
        </el-tab-pane>
      </el-tabs>
      ...
      <el-form :model="accountForm" label-width="80px" :rules="rules" ref="accountFormRef">
        ...
        <div v-if="sseConnecting" class="qrcode-container">
          ...
          <div v-else-if="loginStatus === '500'" class="error-wrapper">
            ...
            <span>{{ loginErrorMessage || '添加失败，请稍后再试' }}</span>
          </div>
        </div>
      </el-form>
      <template #footer>
